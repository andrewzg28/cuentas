...
//CONTADOR DE DATOS
//TOTAL DE CUENTAS
$sql = "SELECT COUNT(*) AS total FROM cuentas WHERE Cuenta_de=2";
$result = mysqli_query($conn,$sql);
while($row = $result->fetch_assoc()) {
    $andres = $row["total"];
}
//CAPITAL INVERTIDO
$sql2 = "SELECT SUM(Capital) AS capital FROM cuentas WHERE Cuenta_de=2";
$result2 = mysqli_query($conn,$sql2);
while($row2 = $result2->fetch_assoc()) {
    $capital = $row2["capital"];
}
//INTERES A PAGAR
$sql3 = "SELECT SUM(interes) AS interes FROM cuentas WHERE Cuenta_de=2";
