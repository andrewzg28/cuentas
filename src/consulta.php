...
if($view=="menu"){
    $sql = "SELECT Nombre,Apellido,Cuenta_de, id_cpropia FROM cuentas ORDER BY Cuenta_de, Nombre ASC";
    ...
    if(isset($_REQUEST["btn_buscar"])){
        ...
        //echo $id_cuenta;
        $tabla = '<center><b>Resultado</b><br><br>
        <table class="table-hover table-responsive-xl">
            <tbody>
            <form method="post" action="busca_cuenta.php">';
                while($row1 = $result1->fetch_assoc()) {
                    ...
                    <tr>
                        <td colspan="4"><button type="submit" id="busca_cuenta" name="busca_cuenta" class="btn btn-success btn-block">Detalles <i class="fa-solid fa-arrow-right"></i></button></td>
                    </tr>
            </form>';
            }
        $tabla .= '</tbody>
